...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.wm2 = global.namabot + ' ' + global.owner
        global.wm = '𝖦𝖺𝗇𝗌𝖡𝖮𝖳𝗓-𝙑2'
        global.colong1 = 'Ciss 📸'
        global.colong2 = 'D͜͡e͜͡f͜͡f͜͡t͜͡i͜͡ G͜͡a͜͡n͜͡s͜͡'
        global.kontak2 = [
        ['6289509960601', '𝙂𝙖𝙣𝙨𝘽𝙊𝙏𝙯-𝙑2', '𝐁𝐚𝐧𝐭𝐮 𝐃𝐨𝐧𝐚𝐬𝐢 𝐋𝐚𝐡 𝐁𝐚𝐧𝐠.\n𝐆𝐚𝐤 𝐃𝐨𝐧𝐚𝐬𝐢 𝐉𝐮𝐠𝐚 𝐆𝐚𝐩𝐚𝐩𝐚 𝐓𝐚𝐩𝐢 𝐁𝐚𝐧𝐭𝐮 𝐒𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐒𝐮𝐫𝐯𝐢𝐯 𝐎𝐅𝐂 𝐲𝐚𝐡𝐡😁', true],
        ['6289509960601', await this.getName('6281361281833@s.whatsapp.net'), '𝑴𝒚 𝑻𝒆𝒂𝒎', true], 
        ]
            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        let pp = 'https://telegra.ph/file/e8c5f14b2baeb0f0c95b6.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                                (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0]).replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc)
                            await this.send3TemplateButtonImg(id, action === 'add' ? pp : pp, text, wm, action === 'add' ? 'Welcome' : 'GoodBye', action === 'add' ? '.intro' : 'FokusID')
                        }
                    }
                }
                break
